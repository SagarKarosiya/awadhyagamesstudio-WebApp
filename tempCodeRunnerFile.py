...
def add_game():
    ...
    if request.method == "POST":
        ...
        featured = True if request.form.get("featured") == "1" else False
    ...
    # Pass game=None so template doesn’t break
    return render_template("admin/add_game.html", game=None)
...
def add_screenshot(game_id):
    ...
    if file and file.filename != "":
        ...
        file.save(path)
        shot = Screenshot(game_id=game_id, image=path)
        db.session.add(shot)
        db.session.commit()
